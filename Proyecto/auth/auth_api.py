...
# Enrutador principal del módulo de autenticación
...
def iniciar_sesion(form_data: OAuth2PasswordRequestForm = Depends()):
    """
    Recibe: OAuth2PasswordRequestForm con username y password.
    Función: Verifica credenciales, genera un token JWT y guarda sesión.
    """
    ...
    token = crear_token_acceso(
        {"sub": usuario["username"], "tipo": usuario["tipo"]}
    )
...
def cerrar_sesion(
    ...
    """
    Finaliza la sesión activa del usuario autenticado eliminando el token.
    """
...
@router.delete(
    "/borrar_usuario/{username}", summary="Eliminar usuario (solo admin)"
)
def borrar_usuario(
    username: str,
    usuario_actual: UsuarioPublico = Depends(obtener_usuario_actual)
):
    """Elimina un usuario de la base de datos (solo administradores)."""
    ...
    if not exito:
        raise HTTPException(
            status_code=500, detail="No se pudo eliminar el usuario."
        )
